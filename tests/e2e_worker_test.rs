...
/// Test using the high-level Circuit builder + Analysis -> netlist/control flow.
#[test]
fn circuit_builder_to_worker() {
    use piperine_core::prelude::*;

    let ckt = Circuit::new("Builder Test")
        .vdc("1", "in", GND, 10.0)
        .resistor("1", "in", "mid", "2k")
        .resistor("2", "mid", "out", "3k")
        .resistor("3", "out", GND, "5k");

    let analysis = OpAnalysis::new();

    let netlist_lines = ckt.to_netlist_lines();
    let control_commands = analysis.to_control_commands();

    eprintln!("Netlist:");
    for l in &netlist_lines { eprintln!("  {l}"); }
    eprintln!("Control:");
    for l in &control_commands { eprintln!("  {l}"); }
    ...
    let _: WorkerToMain = read_msg(&mut stdout); // Ready
    ...
    match result {
        WorkerToMain::SimulationComplete { plots, .. } => {
            ...
            eprintln!("Builder test: {} plots", plots.len());
        }
        WorkerToMain::Error { message } => panic!("builder test error: {message}"),
        other => panic!("unexpected: {other:?}"),
    }
