...
impl Model for DiodeShockleyModel {
    ...
    fn update(&self, diode: &mut Diode) -> crate::error::Result<()> {
        ...
        let v_new = diode.v_guess.value.re;      // Raw 5V guess
        let v_old = diode.v_linearized.value.re; // Safe 0.16V from previous step
        ...
        // 2. Critical Voltage
        let vcrit = nvt * (nvt / (std::f64::consts::SQRT_2 * is.re)).ln();
        ...
        let vd_limited = if v_new > vcrit && (v_new - v_old).abs() > (2.0 * nvt) {
            ...
            if v_new < v_old - 2.0 * nvt { v_old - 2.0 * nvt } else { v_new }
        } else {
            v_new
        };
        ...
        let id = is.re * (exp_term - 1.0);
        let gd = (is.re / nvt) * exp_term;
