...
fn capacitor_ddt_lowers_to_charge_function() {
    ...
    // susceptance ωC (AC).
    let mut qjac = [0.0; 4];
    dev.eval_charge_jacobian(&v, &params, &vec![0.0; dev.num_state_slots()], &[], &SimCtx::default(), &mut qjac);
...
fn nonlinear_ddt_charge_uses_chain_rule() {
    ...
    let mut q = [0.0; 2];
    dev.eval_charge(&[vv, 0.0], &params, &vec![0.0; dev.num_state_slots()], &[], &SimCtx::default(), &mut q);
    assert!(close(q[0], cj * vv * vv), "Q = {}, want {}", q[0], cj * vv * vv);

    // dQ/dV = 2·Cj·V
    let mut qjac = [0.0; 4];
    dev.eval_charge_jacobian(&[vv, 0.0], &params, &vec![0.0; dev.num_state_slots()], &[], &SimCtx::default(), &mut qjac);
...
fn idt_operator_is_recognised_and_lowered_to_companion() {
    ...
    assert!(!body.states.is_empty(), "idt must allocate a state var in the IR");
