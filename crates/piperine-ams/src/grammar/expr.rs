...
impl<'a> Parser<'a> {
    ...
    fn parse_inside(&mut self, lhs: Expr) -> PResult<Expr> {
        ...
        while !self.at(&Tok::RBrace) && !self.at_end() {
            if self.eat(&Tok::LBrack) {
                ...
                terms.push(Self::range_term(&lhs, lo, hi));
            } else {
                let v = self.expr()?;
    ...
    /// A trailing ASCII letter marks an SI scale factor; otherwise plain decimal/exponent.
