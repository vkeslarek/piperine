...
/// Signal domain of a discipline: continuous (KCL/KVL, `electrical`) or
/// discrete (logic event-driven, `logic`). Matches Verilog-AMS `domain` attribute.
...
pub struct DisciplineDecl {
    ...
    pub attrs: Vec<Attr>,
    pub name: Name,
    /// `domain continuous` / `domain discrete` parsed from the body.
    /// `None` means the discipline body did not specify a domain explicitly.
...
/// ungram: `ModuleItem = PortDecl | NetDecl | AnalogBehaviour | Function
///                       | BranchDecl | VarDecl | ParamDecl | AliasParam`
/// Plus `Instance` (Piperine structural extension).
#[derive(Debug, Clone)]
pub enum ModuleItem {
    /// Port re-declaration inside the module body (`input electrical A;`).
    PortDecl(PortDecl),
    NetDecl(NetDecl),
