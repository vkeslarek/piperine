...
use crate::lexer::{Lexer, Lexed, Tok};
...
impl Preprocessor {
    ...
    pub fn define(&mut self, name: &str, body: &str) {
        let body = Lexer::tokenize(body).unwrap_or_default();
    ...
    fn dir_define(&mut self, tokens: &[Lexed], i: usize) -> Result<usize, String> {
        // i points at `define
        let mut j = i + 1;
        let (name, name_end) = Self::expect_ident(tokens, j)?;
        j += 1;
            ...
            && tokens[j].start == name_end
        {
            ...
            let mut ps = Vec::new();
            loop {
                let (p, _) = Self::expect_ident(tokens, j)?;
                ps.push(p);
                j += 1;
    ...
    fn dir_include(&mut self, tokens: &[Lexed], i: usize) -> Result<usize, String> {
        let next = Self::skip_to_newline(tokens, i + 1);
        if !self.active() {
            return Ok(next);
        }
    ...
    fn include_file(&mut self, rel: &str) -> Result<(), String> {
        ...
        let toks = Lexer::tokenize(&text).map_err(|e| format!("{}: {e}", resolved.display()))?;
    ...
    fn expand_use(
        ...
        let args: Vec<Vec<Lexed>> = if mac.params.is_some() {
            ...
            let (args, end) = Self::parse_args(tokens, k)?;
            next = end;
            args
        } else {
            Vec::new()
            ...
            match &bt.tok {
                Tok::Ident(id) if subst.contains_key(id.as_str()) => {
                    for a in subst[id.as_str()] {
                        sub.push(Self::respan(a, site));
                    }
                }
                _ => sub.push(Self::respan(bt, site)),
            }
        }
                ...
                Tok::Tick(name) => {
                    ...
                    let (args, next) = if mac.params.is_some() {
                        ...
                        if matches!(toks.get(k).map(|t| &t.tok), Some(Tok::LParen)) {
                            let (a, e) = Self::parse_args(toks, k)?;
                            (a, e)
                        } else {
                            (Vec::new(), i + 1)
                        }
