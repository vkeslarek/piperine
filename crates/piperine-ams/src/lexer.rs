...
impl<'a> Lexer<'a> {
    ...
    pub fn tokenize(src: &str) -> Result<Vec<Lexed>, String> {
        let all = Self::tokenize_with_comments(src)?;
    ...
    fn run(mut self) -> Result<Vec<Lexed>, String> {
        while self.pos < self.src.len() {
            ...
            match c {
                ...
                _ if Self::is_ident_start(c) => self.ident(start),
    ...
    fn tick(&mut self, start: usize) -> Result<(), String> {
        ...
        while self.pos < self.src.len() && Self::is_ident_continue(self.peek(0)) {
            self.pos += 1;
        }
    ...
    fn eat_time_unit(&mut self) -> bool {
        for unit in [b"fs" as &[u8], b"ps", b"ns", b"us", b"ms"] {
            if self.peek(0) == unit[0]
                && self.peek(1) == unit[1]
                && !Self::is_ident_continue(self.peek(2))
            {
                self.pos += 2;
                return true;
            }
        }
        if self.peek(0) == b's' && !Self::is_ident_continue(self.peek(1)) {
            self.pos += 1;
            return true;
        }
        false
