//! `AnalogInstance` — the analog half of a device instance — together with the
//! two contracts its capabilities share: `LoadCtx` (the read-only view of
//! kernel, netlist references and banks handed to each stamp) and `Stamps`
//! (the per-capability stamp contribution `load_dc`/`load_transient` fold).
...
pub(super) struct LoadCtx<'a> {
    pub(super) kernel: &'a AnalogKernel,
    pub(super) node_refs: &'a [Option<AnalogReference>],
    pub(super) params: &'a [f64],
    pub(super) state: &'a [f64],
    pub(super) vars: &'a [f64],
    pub(super) sim: &'a SimCtx,
}

impl LoadCtx<'_> {
    /// The netlist reference for a kernel terminal node (`None` = ground).
    pub(super) fn terminal_ref(&self, node: NodeId) -> Option<AnalogReference> {
        self.kernel
            .terminals()
            .iter()
            .position(|&t| t == node)
...
pub struct AnalogInstance {
    pub(super) kernel: Arc<AnalogKernel>,
    /// Per-terminal netlist references (`None` = ground).
    pub(super) node_refs: Vec<Option<AnalogReference>>,
...
impl AnalogInstance {
    ...
    pub(super) fn num_terminals(&self) -> usize {
        self.node_refs.len()
    }
    ...
    /// The netlist reference for a kernel terminal node (None = ground).
    pub(super) fn terminal_ref(&self, node: crate::resolve::NodeId) -> Option<AnalogReference> {
        self.kernel
            .terminals()
            .iter()
            .position(|&t| t == node)
    ...
    fn sync_sim(&mut self, context: &Context, analysis: Analysis) {
        ...
        self.sim.current_analysis = crate::device::analysis_code(analysis);
