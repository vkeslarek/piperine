//! [`PiperineDevice`] — one device instance as the solver sees it: parameter
//! values, operator state, register banks and netlist references, wrapped as
//! a single flat `Element` implementing both domains (MD-01: no `Device`
//! wrapper, no downcast, no per-capability trait object crosses the ABI).
...
use super::analog::AnalogInstance;
use super::digital::DigitalInstance;

/// One device instance: the mixed-signal `Element` the solver drives.
pub struct PiperineDevice {
    pub(super) label: String,
    pub(super) analog: Option<AnalogInstance>,
    pub(super) digital: Option<DigitalInstance>,
    ...
    pub(super) meta: piperine_lang::pom::IntrospectionMeta,
...
pub(super) fn analysis_code(analysis: Analysis) -> u64 {
    analysis as u64
}
