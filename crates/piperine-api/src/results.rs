...
pub use crate::model::InstanceView;
...
impl OpResult {
    ...
    pub fn instance(&self, label: &str) -> Result<InstanceView, Error> {
        let (name, vars) = self
            .opvars
            .get_key_value(label)
        ...
        Ok(InstanceView::catalog(name, vars, model, terminals, observables, params))
    }
