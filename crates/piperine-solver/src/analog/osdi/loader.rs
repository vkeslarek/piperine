...
use std::sync::Arc;
use std::path::Path;
use crate::analog::osdi::ffi::OsdiDescriptor;
...
impl OsdiLib {
    pub fn load(path: &Path) -> Result<Arc<Self>, libloading::Error> {
        #[cfg(unix)]
        unsafe {
            // Load libm into the global namespace to resolve math functions (pow, exp, etc.) for the .osdi library.
            let _ = libloading::os::unix::Library::open(Some("libm.so.6"), libloading::os::unix::RTLD_NOW | libloading::os::unix::RTLD_GLOBAL);
        }
